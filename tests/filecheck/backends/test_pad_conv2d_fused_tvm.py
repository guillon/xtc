...
# CHECK-NEXT:  sch[I_F0].compute_at(sch[O], f)
# CHECK-NEXT:  
# CHECK-NEXT:  # from tvm.script import ir as I
# CHECK-NEXT:  # from tvm.script import tir as T
# CHECK-NEXT:  
...
# CHECK-NEXT:      def main(_0: T.Buffer((1, 8, 8, 3), "float32"), _1: T.Buffer((5, 5, 3, 16), "float32"), conv_: T.Buffer((1, 4, 4, 16), "float32")):
# CHECK-NEXT:          T.func_attr({"from_legacy_te_schedule": T.bool(True), "tir.noalias": T.bool(True)})
# CHECK-NEXT:          pad_ = T.allocate([75], "float32", "global")
# CHECK-NEXT:          for h, w, f in T.grid(4, 4, 16):
# CHECK-NEXT:              pad__1 = T.Buffer((75,), data=pad_)
# CHECK-NEXT:              for hi, wi, ci in T.grid(5, 5, 3):
# CHECK-NEXT:                  cse_var_3: T.int32 = wi * 3
# CHECK-NEXT:                  cse_var_2: T.int32 = hi // 2 + h
# CHECK-NEXT:                  cse_var_1: T.int32 = wi // 2 + w
# CHECK-NEXT:                  _0_1 = T.Buffer((192,), data=_0.data)
# CHECK-NEXT:                  pad__1[hi * 15 + cse_var_3 + ci] = T.if_then_else(1 <= cse_var_2 and cse_var_2 < 5 and 1 <= cse_var_1 and cse_var_1 < 5, _0_1[h * 48 + hi * 24 + w * 6 + cse_var_3 + ci - 54], T.float32(0.0))
# CHECK-NEXT:              conv__1 = T.Buffer((256,), data=conv_.data)
# CHECK-NEXT:              conv__1[h * 64 + w * 16 + f] = T.float32(0.0)
# CHECK-NEXT:              for r, s, c in T.grid(5, 5, 3):
# CHECK-NEXT:                  cse_var_4: T.int32 = h * 64 + w * 16 + f
# CHECK-NEXT:                  _1_1 = T.Buffer((1200,), data=_1.data)
# CHECK-NEXT:                  conv__1[cse_var_4] = conv__1[cse_var_4] + pad__1[r * 15 + s * 3 + c] * _1_1[r * 240 + s * 48 + c * 16 + f]
# CHECK-NEXT:  CODE: 0
